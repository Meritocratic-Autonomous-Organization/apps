...
const TokenRequestVesting = artifacts.require('TokenRequestVesting')
...
contract('TokenRequestVesting', ([rootAccount, ...accounts]) => {
  let requestableToken, tokenRequestBase, tokenRequest, tokenManager, tokenManagerBase, mockErc20, vaultBase, vault
  let FINALISE_TOKEN_REQUEST_ROLE, MINT_ROLE, ASSIGN_ROLE, ISSUE_ROLE, SET_TOKEN_MANAGER_ROLE, SET_VAULT_ROLE, MODIFY_TOKENS_ROLE
  let dao, acl
  ...
  before('deploy base apps', async () => {
    tokenRequestBase = await TokenRequestVesting.new()
  ...
  beforeEach('deploy dao and token request', async () => {
    ...
    tokenRequest = await TokenRequestVesting.at(deployedContract(newTokenRequestAppReceipt))
  ...
  xdescribe('initialize(address _tokenManager, address _vault, address[] _acceptedDepositTokens)', async () => {
  ...
  describe('initialize(address _tokenManager, address _vault, address[] _acceptedDepositTokens)', () => {
    ...
    xit('sets correct variables', async () => {
    ...
    xdescribe('setTokenManager(address _tokenManager)', () => {
      beforeEach(async () => {
    ...
    xdescribe('setVault(address _vault)', () => {
      beforeEach(async () => {
    ...
    xdescribe('addToken(address _token)', async () => {
      beforeEach(async () => {
      ...
      xit('cannot add more than max tokens', async () => {
        const maxTokens = await tokenRequest.MAX_ACCEPTED_DEPOSIT_TOKENS()
    ...
    xdescribe('removeToken(address _token)', async () => {
      beforeEach(async () => {
        await acl.createPermission(accounts[1], tokenRequest.address, MODIFY_TOKENS_ROLE, rootAccount)
      })
    ...
    describe('createTokenRequest(address _depositToken, uint256 _depositAmount, uint256 _requestAmount, string _reference)', () => {
      beforeEach(async () => {
        await acl.createPermission(rootAccount, tokenManager.address, ISSUE_ROLE, rootAccount)    
        await tokenManager.issue(300)        
      })            
      ...
      it('shoul not create a token request if the amount of tokens on the tokenManager balance is less than required in a request', async () => {
	await assertRevert(
          tokenRequest.createTokenRequest(ETH_ADDRESS, ROOT_ETHER_AMOUNT, 400, REFERENCE, {
             value: ROOT_ETHER_AMOUNT,
        }),
        'TOKEN_REQUEST_NOT_ENOUGH_TOKENS_AT_TOKEN_MANAGER'
	)
      })      
    ...
    describe('finaliseTokenRequest(uint256 _tokenRequestId)', () => {
      ...
      beforeEach(async () => {
        ...
        await acl.createPermission(tokenRequest.address, tokenManager.address, ASSIGN_ROLE, rootAccount)
